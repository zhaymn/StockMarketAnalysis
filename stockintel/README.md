...
└── frontend/
    └── src/
        ...
        ├── components/       controls, prediction, charts, analytics, evidence, news
...
This lives alongside an earlier Streamlit + TensorFlow LSTM project at the
repository root, which remains fully functional and untouched. The market
provider abstraction here is adapted from that project's `src/markets` package —
including its verified finding that NSE's own site blocks automated requests
(HTTP 403/503 behind a WAF), which is why Indian listings are sourced through
Yahoo Finance's search API.
